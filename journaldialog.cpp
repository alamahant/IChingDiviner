...
void JournalDialog::displayEntries(const QVector<JournalEntry>& entries)
{
    ...
    for (const JournalEntry& entry : entries) {
        ...
        // Show hexagram info if this is a saved reading entry
        if (entry.sourceHexagram > 0) {
            if (entry.modifiedHexagram > 0 && entry.modifiedHexagram != entry.sourceHexagram) {
                html += QString("<b>Hexagram %1 → %2</b>").arg(entry.sourceHexagram).arg(entry.modifiedHexagram);
            } else {
                html += QString("<b>Hexagram %1</b>").arg(entry.sourceHexagram);
            }
...
void JournalDialog::onSaveEntry()
{
    ...
    entry.sourceHexagram = 0;       // Manual entry has no hexagram
    entry.modifiedHexagram = 0;     // Manual entry has no hexagram
