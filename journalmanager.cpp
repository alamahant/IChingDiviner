...
void JournalManager::addEntry(int sourceHexagram, int modifiedHexagram,
                              const QString& deckName,
                              const QString& filePath, const QString& notes)
{
    JournalEntry entry;
    entry.timestamp = QDateTime::currentDateTime();
    entry.sourceHexagram = sourceHexagram;
    entry.modifiedHexagram = modifiedHexagram;
...
void JournalManager::save()
{
    ...
    for (const JournalEntry& entry : m_entries) {
        ...
        obj["sourceHexagram"] = entry.sourceHexagram;
        obj["modifiedHexagram"] = entry.modifiedHexagram;
...
void JournalManager::load()
{
    ...
    for (const QJsonValue& val : doc.array()) {
        ...
        entry.sourceHexagram = obj["sourceHexagram"].toInt();
        entry.modifiedHexagram = obj["modifiedHexagram"].toInt();
...
{
    QString path = Constants::journalDirPath;
